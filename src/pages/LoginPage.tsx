...
import { MapPin, User, Lock, Eye, EyeOff, ScanLine, Shield } from "lucide-react";
...
export function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const data = await login({ login: loginId, password });
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      {/* Left panel - branding */}
      <div className="lg:w-1/2 bg-gradient-to-br from-primary-900 via-primary-800 to-primary-950 text-white flex flex-col justify-center px-8 py-12 lg:px-16 lg:py-20 relative overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-10 left-10 w-64 h-64 rounded-full bg-primary-400 blur-3xl" />
          <div className="absolute bottom-10 right-10 w-72 h-72 rounded-full bg-primary-500 blur-3xl" />
        </div>
        <div className="relative z-10 max-w-md">
          <div className="flex items-center gap-3 mb-10">
            ...
            <div>
              <h1 className="text-xl font-bold">Contrôle Présence</h1>
              <p className="text-primary-200 text-sm">Vérification terrain</p>
            </div>
          </div>
          ...
          <p className="text-primary-200 text-base leading-relaxed mb-8">
            Scannez un QR code, géolocalisez, confirmez. Une solution fiable et
            sécurisée pour garantir le passage de vos équipes sur les points de vente.
          </p>
          <div className="space-y-3">
            {[
              { icon: ScanLine, text: "Scan QR Code via caméra du téléphone" },
              { icon: MapPin, text: "Validation GPS à moins de 50 mètres" },
      ...
      <div className="lg:w-1/2 flex items-center justify-center px-8 py-12 lg:px-16 bg-gray-50">
        <div className="w-full max-w-sm">
          ...
          <p className="text-gray-500 text-sm mb-8">Saisissez vos identifiants pour accéder à votre espace</p>
